  ...
  return (
    <Group direction="column" spacing="xs">
      <Tooltip label="Hide namespace prefixes from column labels." multiline withArrow w={220} transitionProps={{ duration: 300 }}>
        <Switch
          checked={hideColumns}
          onChange={(e) => dispatch(setHideNamespacePrefixColumns(e.currentTarget.checked))}
          label={hideColumns ? 'Hide Prefix in Columns' : 'Show Prefix in Columns'}
        />
      </Tooltip>
      <Tooltip label="Also hide prefixes in cell values." multiline withArrow w={220} transitionProps={{ duration: 300 }}>
        <Switch
          checked={hideCells}
          onChange={(e) => dispatch(setHideNamespacePrefixCells(e.currentTarget.checked))}
          label={hideCells ? 'Hide Prefix in Cells' : 'Show Prefix in Cells'}
        />
      </Tooltip>
    </Group>
  );
